...
export default function Paytm({ paymentGateways, setPaymentGateways }: any) {


    const paytm = paymentGateways.find((pg: any) => pg.payment_gateway_name == "Paytm")
    ...
    useEffect(() => {
        if (!crmToken) navigate('/')
            ...
            setParams({
                payment_gateway_name: 'Paytm',
                status: paytm.status,
                field_1: paytm.field_1,
                field_2: paytm.field_2,
                field_3: paytm.field_3,
            })
        }
        dispatch(setPageTitle('Settings | Payments | Paytm'))
    }, [paytm])
    ...
    const AddEmployee = async (data: any) => {
        ...
        try {
            ...
            if (response.data.status == 'success') {
                ...
                const index = paymentGateways.findIndex((a: any) => a.payment_gateway_name == 'Paytm');
                if (index >= 0) {
                    const newPaymentGateways = [...paymentGateways];
                    newPaymentGateways[index] = response.data.paytm;
                    console.log("new ", newPaymentGateways)
                    setPaymentGateways(newPaymentGateways)
                } else {
                    const newPaymentGateways = [...paymentGateways, response.data.paytm];
                    setPaymentGateways(newPaymentGateways)
                }
            }
