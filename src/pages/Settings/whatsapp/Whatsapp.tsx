...
const Whatsapp = () => {
    ...
    useEffect(() => { fetchDropdowns(); }, [page, search])
    ...
    const fetchDropdowns = async () => {
        ...
        try {
            ...
            const response = await axios({
                method: 'get',
                url: APIurl + "whatsapptemplates?page=" + page + "&pageSize=" + pageSize + "&search=" + search + "&whatsappConfiguration=" + a,
                headers: {
            ...
            if (response.data.status == "success") {
                ...
                if (response.data.whatsappConfiguration) setWhatsappConfiguration(response.data.whatsappConfiguration)
            }
    ...
    function handleDownloadExcel() {
        ...
        downloadExcel({
            ...
            tablePayload: {
                header,
                body: dropdowns.map((sms: any, index) => ({
                    a: index + 1,
                    c: sms.template_name,
                    d: sms.template,
    ...
    const exportTable = (type: any) => {
        ...
        let records = dropdowns.map((sms: any, index) => ({
            "#": index + 1,
    ...
    const validate = () => {
        ...
        if (!params.template_name) {
            errors = { ...errors, template_name: "Dropdown Value is required" };
        }
        console.log(errors);
        setErros(errors);
        ...
        Swal.fire({
            ...
            if (result.value) {
                try {
                    const response = await axios({
                        method: 'delete',
                        url: APIurl + 'whatsapptemplates/' + dropdowns.id,
                        headers: {
                            'Content-Type': 'application/json',
                            Authorization: "Bearer " + crmToken,
                        },
    ...
    const updateStatus = (id: any) => {
        const dropdown: any = dropdowns.find((d: any) => d.id == id);
        AddDropdown({ ...dropdown, status: dropdown.status == "1" ? '0' : '1' })
    }
    ...
    return (
        <div className='w-full'>
            <ConfigurationDrawer showDrawer={showDrawer} setShowDrawer={setShowDrawer} whatsappConfiguration={whatsappConfiguration} />
                    ...
                    <div className="flex flex-col h-full">
                        <div className='panel'>
                            <div className='flex items-center justify-between mb-1'>
                                <h5 className="font-semibold text-lg dark:text-white-light">Whastapp Template List</h5>
                                            ...
                                            },
                                            {
                                                ...
                                                render: ({ template }) => (
                                                    <div className="w-[100px]">
                                                        <Tippy content={template} placement="bottom">
                                                            <div className="font-semibold truncate">{template}</div>
                                                        </Tippy>
                                                    </div>
                                                ),
                                            },
